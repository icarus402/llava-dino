...
class OpenCLIPVisionTower(torch.nn.Module):
    def __init__(self, vision_tower, args, delay_load=False, vision_tower_config=None, vision_tower_checkpoint=None):
        ...
        if os.path.exists(vision_tower_config):
            ...
            cache_file = "/home/hanruize/llava-rad/biomedclipcxr_518.json"
    ...
    def load_model(self):
        '''if self.vision_tower_checkpoint:
            if not os.path.exists(self.vision_tower_checkpoint):
                ...
                def load_from_hf(repo_id=LLAVARAD_HF_REPO, filename="",subfolder=None):
                    cache_file = "/home/hanruize/llava-rad/biomedclipcxr_518_checkpoint.pt"
                    return cache_file
        ...
        local_model_path = '/home/hanruize/rad-dino'
        processor = AutoImageProcessor.from_pretrained(local_model_path, local_files_only=True)
        self.vision_tower = AutoModel.from_pretrained(local_model_path, local_files_only=True)
    ...
    def feature_select(self, image_forward_outs):
        ...
        if self.select_feature == 'patch':
            ...
            print(f"视觉特征形状: {image_features.shape}")
